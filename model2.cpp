...
int bst = 7, usr = 1, cel = 7 ; //no. of base stations, users and cells (assuming 1bst per cell)
...
void Betav(void)
{
	float base_loc[7][2] = {{0,0},{0,2},{1,1.73}, {1, -1.73},{-1,-1.73},{0,-2},{-1,1.73}};
	float user_loc[7][2] = {{0,0.5},{0.2,0.9}, {0,2.5}, { -0.2,1.2}, {1.25,2.165},  {1.25,-2.165}, {-1.25, -2.165}};	
	float D = 0; 
	int cl =0;
	ivec locations_usr ;
	int LO= -1 , HI = 1 ;
	...
	for(int i = 0; i < bst; i++)
	{
		...
		{				
			for(int j = 0 ; j <2; j++)
			{	
				D = sqrt((pow((base_loc[i][0]-user_loc[o][0]),2)) + (pow((base_loc[i][1]-user_loc[o][1]),2)));
	...
	//	{
				...
				{				
					...
					{	
						...
						if((r==0)&&(q==0))
							...
							power1 = 0.8 ;
						received_symbols[r][q][p][b] = sqrt(beta[r][0][0])*divid[r][0][b]*w[r][b]*transmitted_symbols[b]*power1;
			...
			{			
				for(int r = 0 ; r < bst ; r++)
				{
					for(int q = 0 ; r < usr ; r++)
				{
						rx[0][0][b] += received_symbols[r][q][0][b];
				}
				}
